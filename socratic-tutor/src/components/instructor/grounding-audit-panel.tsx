...
export function GroundingAuditPanel({ sessionId }: { sessionId: string }) {
  ...
  if (!data) return <p className="text-sm text-[var(--dim-grey)]">Loading grounding records…</p>;
  ...
  return (
    <div className="space-y-8">
      ...
      <section>
        <h2 className="font-serif text-3xl text-[var(--charcoal)]">Tutor response audit</h2>
        <div className="mt-4 space-y-4">
          {data.groundings.length === 0 ? <Empty text="No tutor responses have been audited yet." /> : data.groundings.map((item) => (
            <article key={item.id} className="minerva-card p-6">
              <div className="flex flex-wrap items-center justify-between gap-2">
                ...
                <span className="rounded-full bg-[rgba(17,120,144,0.08)] px-3 py-1 text-xs font-semibold uppercase tracking-wide text-[var(--teal)]">{item.status.replace(/_/g, " ")}</span>
              </div>
      ...
      <section>
        ...
        <div className="mt-4 space-y-4">
          {data.protectionEvents.length === 0 ? <Empty text="No protected-answer attempts have influenced coaching." /> : data.protectionEvents.map((item) => (
            <article key={item.id} className="minerva-card p-6">
