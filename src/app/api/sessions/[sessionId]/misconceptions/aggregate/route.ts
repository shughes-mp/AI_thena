...
function getDetectionConfidence(
  ...
  const values = records
    .map((record) => record.evidenceSignal?.confidenceLevel || record.confidence)
    .filter(
      (value): value is "high" | "medium" | "low" =>
...
export async function GET(
  ...
  try {
        ...
        select: {
          ...
          evidenceSignal: {
            select: {
              ...
              citations: {
                where: { citationType: "source_passage" },
                select: {
                  id: true,
                  sourceFilename: true,
                  passageId: true,
    ...
    if (usableMisconceptions.length === 0) {
      ...
      semanticGroups.forEach((group, index) => {
        ...
        clusters.push({
          ...
          records: group.misconceptions.map((record) => ({
            ...
            evidenceSignalId: record.evidenceSignal?.id ?? null,
            reviewState:
              (record.evidenceSignal?.status as
                | "provisional"
                | "approved"
                | "revised") ?? "legacy_unversioned",
            sourceCitations: record.evidenceSignal?.citations ?? [],
          })),
          ...
          resolutionConfidence:
            ...
            group.misconceptions.find((record) => record.resolutionConfidence)
              ...
              : undefined,
          detectionConfidence: getDetectionConfidence(group.misconceptions),
        });
      });
    }
    ...
    const sessionStats: MisconceptionDashboardStats = {
      totalStudents,
        totalMisconceptions: usableMisconceptions.length,
      avgMisconceptionsPerStudent:
        totalStudents > 0 ? usableMisconceptions.length / totalStudents : 0,
