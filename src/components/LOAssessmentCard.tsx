...
import type { TeachingBriefEvidenceMapItem } from "@/lib/teaching-brief";
...
export function LOAssessmentCard({
  assessment,
  reviewContext,
}: LOAssessmentCardProps) {
    ...
    <div className="rounded-2xl border border-[var(--rule)] bg-white p-4 transition hover:shadow-sm">
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1">
          ...
          <div className="mt-2 flex flex-wrap items-center gap-2">
            ...
            {reviewContext ? (
              <span className="rounded-full border border-[var(--rule)] px-2.5 py-1 text-xs font-semibold capitalize text-[var(--charcoal)]">
                {reviewContext.reviewState.replaceAll("_", " ")}
              </span>
            ) : null}
          </div>
        </div>
      ...
      {expanded && (
        <div className="mt-3 space-y-3 border-t border-[var(--rule)] pt-3 text-sm">
          ...
          {reviewContext ? (
            <div className="rounded-xl border border-[var(--rule)] p-3 text-xs leading-5 text-[var(--dim-grey)]">
              <p className="mb-1 font-medium text-[var(--charcoal)]">
                Opportunity and review context
              </p>
              <p>{reviewContext.opportunityCoverage.summary}</p>
              <p>
                {reviewContext.signalIds.length} linked signal
                {reviewContext.signalIds.length === 1 ? "" : "s"} · {reviewContext.opportunityCoverage.linkedQuestions} linked evidence question
                {reviewContext.opportunityCoverage.linkedQuestions === 1 ? "" : "s"}
              </p>
              <p>Review: {reviewContext.reviewSummary}</p>
              {reviewContext.contradictoryEvidence.length > 0 ? (
                <p>Contradictory evidence: {reviewContext.contradictoryEvidence.join("; ")}</p>
              ) : null}
              {reviewContext.missingEvidence.length > 0 ? (
                <p>Missing evidence: {reviewContext.missingEvidence.join("; ")}</p>
              ) : null}
            </div>
          ) : null}
        </div>
